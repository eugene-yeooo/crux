...
export async function getLogsByUsername(username: string) {
  ...
  const baseLogs = await connection('logs')
    ...
    .select(
      'logs.*',
      'users.username',
      connection.raw(`COALESCE(json_group_array(json_object('mediaId', media.Id,'url', media.url, 'type', media.type, 'caption', media.caption)), '[]') as media`)
...
export async function addLogCanyon(logId: number, canyonData: any, trx: Knex.Transaction) {
  return trx('log_canyons').insert({
    log_id: logId,
    ...canyonData,
  })
}
...
export async function updateLogCanyon(logId: number, canyonData: unknown, trx: Knex.Transaction) {
  return trx('log_canyons').where({ log_id: logId }).update(canyonData)
}
  ...
  try {
    ...
    switch (type) {
      ...
      case 'climb' :
        ...
        break
      case 'canyon' :
        if (data.canyon) await updateLogCanyon(logId, data.canyon, trx)
        break
      //more types here
    }
